...
export function PromptPresetChips(props: {
  ...
  onChoose: (preset: { id: string; text: string }) => void;
}) {
  return (
    <div className="space-y-1 rounded border border-zinc-700 p-2" data-testid="prompt-presets">
      ...
      <div className="flex flex-wrap gap-1">
        {!props.group.presets.length ? <p className="text-xs text-zinc-500">No presets available for this consent context.</p> : null}
        {props.group.presets.map((preset) => (
          <button
            key={preset.id}
            className="rounded border border-zinc-700 px-2 py-1 text-xs text-zinc-300"
            onClick={() => props.onChoose({ id: preset.id, text: preset.text })}
          >
            {preset.label}
          </button>
        ))}
