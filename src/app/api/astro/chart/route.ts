...
export async function POST(req: NextRequest) {
  ...
  if ((!Number.isFinite(lat) || !Number.isFinite(lon)) && data.place) {
    const geo = await geocodePlace(data.place); lat = geo.lat; lon = geo.lon; timezone = geo.timezone;
  }
  if (!Number.isFinite(lat) || !Number.isFinite(lon)) return NextResponse.json({ error: 'Missing coordinates or place lookup failed', ascendantAvailable: false }, { status: 400 });
  ...
  return NextResponse.json({ ...chart, timezone, interpretation: demoAstroInterpretation(chart.placements, chart.aspects, chart.hermeticKeys), packet, mode: data.demoMode ? 'demo' : 'computed' });
}
