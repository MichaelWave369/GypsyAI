...
import { streamOllama, callModel, assertProvider, ensureProviderConfigured } from '@/lib/ai/client';
...
export async function POST(req: NextRequest) {
  ...
  let reading = await callModel(buildGroundedPrompt(packet, 'Gentle'), 0.2, { provider, model });
  if (strictReadingMode) {
    const issues = verifyReading(reading, packet);
    if (issues.length) reading = await callModel(buildRevisionPrompt(reading, packet, issues), 0.1, { provider, model });
  }
