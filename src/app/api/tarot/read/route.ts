...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    let reading = await callModel(buildGroundedPrompt(packet, parsed.style), parsed.temperaturePreset === 'low' ? 0.2 : 0.5, { provider, model: parsed.model });
    if (parsed.accuracyMode) {
      for (let i = 0; i < parsed.passes; i++) {
        const issues = verifyReading(reading, packet);
        if (!issues.length) break;
        reading = await callModel(buildRevisionPrompt(reading, packet, issues), 0.1, { provider, model: parsed.model });
      }
    }
