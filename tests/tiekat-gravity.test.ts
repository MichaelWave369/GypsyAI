...
describe('tiekat gravity bootstrap', () => {
  ...
  it('is deterministic for same input', () => {
    ...
    const a = computeGravityBootstrap({ session: session.state, envelope, verification: verification as any });
    const b = computeGravityBootstrap({ session: session.state, envelope, verification: verification as any });
    expect(a).toEqual(b);
  });

  it('reaches classical limit when information integral is zero', () => {
    const session = buildTiekatSessionState('s3', '', { allowAncestry: false, includeNames: false, hideLivingPersons: true, memoryEnabled: false });
    const envelope = {
      message: '',
  ...
  it('applies consent/redaction penalties', () => {
    const baseSession = buildTiekatSessionState('s4', 'tarot and lineage integration', { allowAncestry: true, includeNames: true, hideLivingPersons: false, memoryEnabled: true });
    ...
    const strictSession = buildTiekatSessionState('s5', 'tarot and lineage integration', { allowAncestry: false, includeNames: false, hideLivingPersons: true, memoryEnabled: false });
    ...
    const strict = computeGravityBootstrap({ session: strictSession.state, envelope: strictEnvelope, verification: verification as any });
